...
contains
    ...
    subroutine compute_damping_profile_ratio
        ...
        call alloc_array(dampratio_left, [nz1 - 1, nz2 + 1])
        call alloc_array(dampratio_right, [nz1 - 1, nz2 + 1])
        call alloc_array(dampratio_top, [nx1 - 1, nx2 + 1])
        call alloc_array(dampratio_bottom, [nx1 - 1, nx2 + 1])

        ! Left boundary
        bindex = 1
        !$omp parallel do private(j)
        do j = nz1 - 1, nz2 + 1
            dampratio_left(j) = damp_ratio(c11(bindex, j), c13(bindex, j), c15(bindex, j), c33(bindex, j), &
                c35(bindex, j), c55(bindex, j), rho(bindex, j), 'x')
        end do
        !$omp end parallel do
        ...
        !$omp parallel do private(j)
        do j = nz1 - 1, nz2 + 1
            dampratio_right(j) = damp_ratio(c11(bindex, j), c13(bindex, j), c15(bindex, j), c33(bindex, j), &
                c35(bindex, j), c55(bindex, j), rho(bindex, j), 'x')
        end do
        !$omp end parallel do
        ...
        do i = nx1 - 1, nx2 + 1
            dampratio_top(i) = damp_ratio(c11(i, bindex), c13(i, bindex), c15(i, bindex), c33(i, bindex), &
                c35(i, bindex), c55(i, bindex), rho(i, bindex), 'z')
        end do
        !$omp end parallel do
        ...
        !$omp parallel do private(i)
        do i = nx1 - 1, nx2 + 1
            dampratio_bottom(i) = damp_ratio(c11(i, bindex), c13(i, bindex), c15(i, bindex), c33(i, bindex), &
                c35(i, bindex), c55(i, bindex), rho(i, bindex), 'z')
        end do
        !$omp end parallel do
    ...
    !
    !> Compute the CPML damping coefficients for isotropic media
    !
    ...
    subroutine compute_cfspml_damping_coef
        ...
        call alloc_array(axii, [nx1, nx2, nz1, nz2])
        call alloc_array(axih, [nx1, nx2, nz1, nz2])
        call alloc_array(axhi, [nx1, nx2, nz1, nz2])
        call alloc_array(axhh, [nx1, nx2, nz1, nz2])

        call alloc_array(bxii, [nx1, nx2, nz1, nz2])
        call alloc_array(bxih, [nx1, nx2, nz1, nz2])
        call alloc_array(bxhi, [nx1, nx2, nz1, nz2])
        call alloc_array(bxhh, [nx1, nx2, nz1, nz2])

        call alloc_array(kxii, [nx1, nx2, nz1, nz2])
        call alloc_array(kxih, [nx1, nx2, nz1, nz2])
        call alloc_array(kxhi, [nx1, nx2, nz1, nz2])
        call alloc_array(kxhh, [nx1, nx2, nz1, nz2])

        call alloc_array(azii, [nx1, nx2, nz1, nz2])
        call alloc_array(azih, [nx1, nx2, nz1, nz2])
        call alloc_array(azhi, [nx1, nx2, nz1, nz2])
        call alloc_array(azhh, [nx1, nx2, nz1, nz2])

        call alloc_array(bzii, [nx1, nx2, nz1, nz2])
        call alloc_array(bzih, [nx1, nx2, nz1, nz2])
        call alloc_array(bzhi, [nx1, nx2, nz1, nz2])
        call alloc_array(bzhh, [nx1, nx2, nz1, nz2])

        call alloc_array(kzii, [nx1, nx2, nz1, nz2])
        call alloc_array(kzih, [nx1, nx2, nz1, nz2])
        call alloc_array(kzhi, [nx1, nx2, nz1, nz2])
        call alloc_array(kzhh, [nx1, nx2, nz1, nz2])
            ...
            do j = nz1, nz2
                do i = nx1, nx2
                    ...
                    if (.not. (i > 1 .and. i < nx .and. j > 1 .and. j < nz)) then
                        ...
                        ! integer-integer
                        call damp_coef(pmlvp, pmlvp, ax, az, bx, bz, kx, kz, xdisti, zdisti, dx, eta_dz_i(j), ratiox, ratioz)
                        axii(i, j) = ax
                        bxii(i, j) = bx
                        kxii(i, j) = kx
                        azii(i, j) = az
                        ...
                        ! integer-half
                        call damp_coef(pmlvp, pmlvp, ax, az, bx, bz, kx, kz, xdisti, zdisth, dx, eta_dz_i(j), ratiox, ratioz)
                        axih(i, j) = ax
                        bxih(i, j) = bx
                        kxih(i, j) = kx
                        azih(i, j) = az
                        ...
                        ! half-integer
                        call damp_coef(pmlvp, pmlvp, ax, az, bx, bz, kx, kz, xdisth, zdisti, dx, eta_dz_i(j), ratiox, ratioz)
                        axhi(i, j) = ax
                        bxhi(i, j) = bx
                        kxhi(i, j) = kx
                        azhi(i, j) = az
                        ...
                        ! half-half
                        call damp_coef(pmlvp, pmlvp, ax, az, bx, bz, kx, kz, xdisth, zdisth, dx, eta_dz_i(j), ratiox, ratioz)
                        axhh(i, j) = ax
                        bxhh(i, j) = bx
                        kxhh(i, j) = kx
                        azhh(i, j) = az
    ...
    end subroutine compute_cfspml_damping_coef
